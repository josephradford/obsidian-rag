# Phase 1: Local RAG Pipeline with MLOps Practices

**Target: April 2026 (~3 weeks)**
**Environment: macOS (Apple Silicon, 8GB RAM) → AWS in Phase 2**

## What you're building

A local Retrieval-Augmented Generation system that queries your Obsidian vault. The RAG pipeline is the vehicle — what makes this a portfolio piece is building it with MLOps discipline from day one.

### Phase 1 scope

| Practice | Phase 1 responsibility | Completed in |
|---|---|---|
| RAG pipeline (ingest, retrieve, generate) | **Build** | Phase 1 |
| FastAPI server | **Build** | Phase 1 (extended in Phase 2) |
| Prompt engineering and versioning | **Build** | Phase 1 |
| MLflow experiment tracking | **Build** | Phase 1 (extended in Phase 3) |
| Structured logging and metrics | **Start** | Phase 2 (CloudWatch) |
| Evaluation dataset and script | **Start** | Phase 3 (full eval framework) |
| Architecture Decision Records | **Start** | Ongoing |
| Data provenance documentation | **Start** | Phase 4 (complete) |
| Model card | **Start** (template) | Phase 4 (complete) |
| Containers, AWS, monitoring | — | Phase 2 |
| Fine-tuning, LLM-as-judge, W&B | — | Phase 3 |
| AIAF mapping, risk register, SOUP | — | Phase 4 |

The goal is: **lay foundations that Phases 2–4 build on**, not do everything at once.
...
|-----------|------|-------------|
| Document source | Obsidian vault (Markdown files) | Real data you know well — you can evaluate quality |
| Ingestion & retrieval | LlamaIndex | Better RAG defaults than LangChain; less boilerplate |
| Embedding model | nomic-embed-text (via Ollama) | Runs locally, good quality, no API costs |
| Vector database | ChromaDB | Embedded (no server), fast to iterate in development |
| LLM | Ollama — Llama 3.2 3B (dev) → larger on AWS | Runs well on 8GB Apple Silicon; swap to 8B+ in Phase 2 |
| API layer | FastAPI | Industry standard for model serving, async, auto-docs |
| Experiment tracking | MLflow (local) | Free, tracks params/metrics/artifacts, AWS cert relevant |
| Logging | Python `logging` with JSON formatter | Sets up clean transition to CloudWatch in Phase 2 |

---

## Prerequisites

### 1. Git repo — set up FIRST

MLOps starts with version control. Create the repo before writing any code.

```bash
mkdir -p ~/projects/obsidian-rag
...
Set up GitHub repo with branch protection on main and a PR template.
...
# Verify:
ollama run llama3.2:3b "Hello, testing"

# TIP: On 8GB RAM, close Chrome/Slack/Teams while developing.
```
...
# Pin dependencies — reproducibility matters
...
│   ├── data_provenance.md      # Data source documentation (started, completed Phase 4)
│   ├── model_card.md           # Model card template (started, completed Phase 4)
...
```bash
mkdir -p src/prompts tests eval docs scripts
